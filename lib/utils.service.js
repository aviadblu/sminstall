...
let runningProcessesDir = path.join(__dirname, "../rp");
...
class Utils {
    ...
    static async cleanTmpFolders() {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let processes = await Utils.fileExists(runningProcessesDir) ? await Utils.readDir(runningProcessesDir) : [];
    ...
    static async addTmpToRunningProcesses(tmpPath) {
        await Utils.ensureDir(runningProcessesDir);
        await fs.ensureFile(path.resolve(runningProcessesDir, path.parse(tmpPath).name));
        return await Promise.resolve();
    }

    static async removeRunningProcesses(tmpPath) {
        return await Utils.deleteDir(path.resolve(runningProcessesDir, path.parse(tmpPath).name));
    }
